import type { ConventionQuestion } from "@/types/conventions";

export const pythonFastapiConventions: ConventionQuestion[] = [
  {
    id: "project-structure",
    category: "Code Organization",
    question: "How should the project be structured?",
    description: "Python projects have several common layout patterns.",
    applicableTo: ["python-fastapi", "python-django"],
    isRequired: true,
    options: [
      {
        id: "domain-driven",
        label: "Domain-driven (app/users/, app/orders/)",
        description: "Organize by business domain. Each domain has its own models, routes, and services.",
        isRecommended: true,
        generatedText: "**Project Structure**: Domain-driven layout. Each domain gets its own package: `app/users/`, `app/orders/`, `app/auth/`. Each package contains `routes.py`, `models.py`, `schemas.py`, `service.py`. Shared utilities go in `app/core/`.",
      },
      {
        id: "layer-based",
        label: "Layer-based (routes/, models/, services/)",
        description: "Organize by technical layer. All routes together, all models together.",
        isRecommended: false,
        generatedText: "**Project Structure**: Layer-based layout. Top-level directories by layer: `routes/`, `models/`, `schemas/`, `services/`, `core/`. Files within each layer are named by domain: `routes/users.py`, `models/users.py`.",
      },
    ],
  },
  {
    id: "api-style",
    category: "API Design",
    question: "What API design conventions should be followed?",
    description: "Consistent API design makes endpoints predictable for frontend consumers.",
    applicableTo: ["python-fastapi", "express"],
    isRequired: true,
    options: [
      {
        id: "rest-resource",
        label: "RESTful resource-based routes",
        description: "Standard REST: GET /users, POST /users, GET /users/{id}. The most common and AI-friendly pattern.",
        isRecommended: true,
        generatedText: "**API Design**: RESTful resource-based routing. Use plural nouns for resources: `GET /api/users`, `POST /api/users`, `GET /api/users/{id}`, `PATCH /api/users/{id}`. Use HTTP status codes correctly (201 for created, 404 for not found). Return consistent response shapes.",
      },
      {
        id: "action-based",
        label: "Action-based routes (/users/create, /users/list)",
        description: "RPC-style routes named after actions. Less conventional but sometimes clearer for complex operations.",
        isRecommended: false,
        generatedText: "**API Design**: Action-based routing. Name routes by their action: `POST /api/users/create`, `GET /api/users/list`, `POST /api/users/{id}/deactivate`. Group related actions under their resource prefix.",
      },
    ],
  },
  {
    id: "python-error-handling",
    category: "Error Handling",
    question: "How should API errors be returned?",
    description: "Consistent error responses make debugging easier for both humans and AI.",
    applicableTo: ["python-fastapi"],
    isRequired: true,
    options: [
      {
        id: "structured-errors",
        label: "Structured error responses with error codes",
        description: "Return JSON with error code, message, and details. Enables programmatic error handling on the frontend.",
        isRecommended: true,
        generatedText: "**Error Handling**: Return structured error responses: `{\"error\": {\"code\": \"USER_NOT_FOUND\", \"message\": \"User with ID 123 not found\", \"details\": {}}}`. Use HTTP exception handlers in FastAPI. Define error codes as an enum. Never expose stack traces in production.",
      },
      {
        id: "http-exceptions",
        label: "FastAPI HTTPException with detail strings",
        description: "Use FastAPI's built-in HTTPException. Simple but less structured.",
        isRecommended: false,
        generatedText: "**Error Handling**: Use FastAPI `HTTPException` for all error responses. Include descriptive `detail` messages. Use appropriate HTTP status codes. Add custom exception handlers for common error types.",
      },
    ],
  },
  {
    id: "state-management",
    category: "State & Data",
    question: "How should application state and DB sessions be managed?",
    description: "Defines how the app handles database connections and global state.",
    applicableTo: ["python-fastapi"],
    isRequired: true,
    options: [
      {
        id: "dependency-injection",
        label: "FastAPI Dependency Injection (Yield)",
        description: "Use FastAPI's Depends() with a generator for DB sessions.",
        isRecommended: true,
        generatedText: "**State Management**: Use FastAPI Dependency Injection. Database sessions managed via `yield` in a `get_db` dependency to ensure proper closing. Use `Annotated` for clean dependency declarations.",
      },
      {
        id: "context-managers",
        label: "Manual Context Managers",
        description: "Explicitly open/close sessions using 'with' blocks.",
        isRecommended: false,
        generatedText: "**State Management**: Use explicit context managers for database sessions and external service clients. Avoid global state; pass sessions explicitly to service functions.",
      },
    ],
  },
  {
    id: "testing-strategy",
    category: "Testing",
    question: "What is the testing strategy for this project?",
    description: "Determines the depth and tools used for automated testing.",
    applicableTo: ["python-fastapi"],
    isRequired: true,
    options: [
      {
        id: "pytest-asyncio",
        label: "Pytest with Integration Tests",
        description: "Use pytest-asyncio and TestClient for functional API testing.",
        isRecommended: true,
        generatedText: "**Testing Strategy**: Use `pytest` with `pytest-asyncio`. Focus on integration tests using FastAPI's `TestClient`. Use an SQLite in-memory database or Dockerized Postgres for a clean test environment. Follow the `tests/` directory convention.",
      },
      {
        id: "no-tests-v1",
        label: "No tests (Speed-focused v1)",
        description: "Skip automated tests to prioritize rapid prototyping.",
        isRecommended: false,
        generatedText: "**Testing Strategy**: No automated tests for version 1. Rely on manual verification and Postman/Insomnia collections for endpoint validation.",
      },
    ],
  },
  {
    id: "api-response-format",
    category: "API Design",
    question: "What should the success response format look like?",
    description: "Standardizing the 'envelope' makes frontend consumption predictable.",
    applicableTo: ["python-fastapi", "express"],
    isRequired: true,
    options: [
      {
        id: "raw-pydantic",
        label: "Raw Pydantic (Standard)",
        description: "Return the model/list directly. Simplest and most standard.",
        isRecommended: true,
        generatedText: "**API Response Format**: Return raw Pydantic models or lists. Let FastAPI handle the JSON serialization of the model directly without a wrapper envelope.",
      },
      {
        id: "enveloped-data",
        label: "Enveloped (data: { ... })",
        description: "Wrap all successful responses in a 'data' key.",
        isRecommended: false,
        generatedText: "**API Response Format**: Use a consistent envelope for all responses: `{\"data\": ..., \"meta\": {}}`. Create a generic `ResponseEnvelope` Pydantic model for all endpoints.",
      },
    ],
  },
  {
    id: "env-variables",
    category: "Configuration",
    question: "How should environment variables be managed?",
    description: "Standardize naming and loading of secrets and config.",
    applicableTo: ["python-fastapi", "python-django"],
    isRequired: true,
    options: [
      {
        id: "pydantic-settings",
        label: "Pydantic Settings (.env files)",
        description: "Use pydantic-settings for type-safe configuration.",
        isRecommended: true,
        generatedText: "**Environment Variables**: Use `pydantic-settings`. Load variables from a `.env` file into a `Settings` class. Access config via a cached `get_settings()` dependency. Use UPPER_CASE naming.",
      },
      {
        id: "os-environ",
        label: "Standard os.environ",
        description: "Basic Python approach using os.getenv().",
        isRecommended: false,
        generatedText: "**Environment Variables**: Use standard `os.getenv()` with fallback defaults. Keep a simple `config.py` that exports these variables as constants.",
      },
    ],
  },
];
